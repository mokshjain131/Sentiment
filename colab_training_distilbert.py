"""
Google Colab Training Script: DistilBERT Model
Lightweight, faster alternative to FinBERT (40% smaller, 60% faster inference)
...
MODEL_OUTPUT_NAME = "distilbert_financial"
...
print(f"Expected speed: 60% faster inference")
print(f"Expected accuracy: 79-81% (~97% of BERT)\n")
...
# Add dropout for regularization (DistilBERT has less regularization by default)
# Increased dropout to combat overfitting
if hasattr(config, 'dropout'):
    config.dropout = 0.3  # Increased from 0.2
if hasattr(config, 'attention_dropout'):
    config.attention_dropout = 0.3  # Increased from 0.2
if hasattr(config, 'seq_classif_dropout'):
    config.seq_classif_dropout = 0.3  # Increased from 0.2
...
print(f"Expected: ~60% faster than BERT-base\n")
